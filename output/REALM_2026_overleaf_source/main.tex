...
strategies~\citep{gao2023survey,rag_reasoning_survey_2025,agentic_rag_survey_2026}; our object is
different---we audit \emph{evaluation practice}. Our contributions are:
...
them operationally: each trigger produces an observable signal, has a downstream consequence, admits a
recovery action, and---critically---maps to a metric that would \emph{detect} it. The rightmost column
...
\emph{use}---Toolformer's call frequency and FrugalRAG's search count/cost~\citep{toolformer,frugalrag}---not
failure or recovery rates. Cost signals are reported unevenly (calls 38\%, tokens 41\%, latency 41\%),
...
by exactly one---Adaptive-RAG's question-complexity router~\citep{adaptive_rag}, whose $\sim$54.5\%
...
path---and these failures need not surface under endpoint-only reporting. This is precisely the concern REALM
...
strategy \emph{and} the minimal metric set that would expose its characteristic failure mode---what to
report \emph{beyond} answer accuracy.
