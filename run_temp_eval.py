...
def sync_data_from_volume(datasets_to_run: list):
    print("\n📦 Syncing trained ablation metrics and results from Modal Volume...")

    modal_bin = [sys.executable, "-m", "modal"]
    sync_targets = []
    for d in datasets_to_run:
        sync_targets.extend([
            f"checkpoints/{d}/temp_ablation",
            f"results/temp_ablation/{d}_temp_ablation_results.csv",
            f"results/temp_ablation/comparison_{d}_temp.json"
        ])
...
LOSSES = ["info_nce_multi", "kl_div"]
TAUS = [0.01, 0.05, 0.07, 0.1, 0.2, 0.5]
...
def run_cloud_ablation(datasets=["2wiki"]):
    # 1. Cloud Directory Setup (Identical to run_modal)
    os.chdir("/root/CRAG")
    ...
    # 2. Lazy Imports (so local execution doesn't crash before pushing to cloud)
    ...
    for ds in datasets:
        log.info(f"========== CLOUD ABLATION TARGET: {ds.upper()} ==========")
        
        # Train all variations sequentially on A10G
        for loss in LOSSES:
            for tau in TAUS:
                log.info(f"--> Ensuring {loss} | tau={tau:g} model is trained...")
                ckpt_path = f"checkpoints/{ds}/temp_ablation/alignment_mlp_{loss}_tau_{tau:g}.pth"
                if not os.path.exists(ckpt_path):
                    log.info(f"Model missing. Triggering training engine for {loss} with tau={tau:g}...")
                    train_ablation(dataset_name=ds, loss_type=loss, tau=tau, epochs=100) 
                    volume.commit()  # Save strictly
                else:
                    log.info(f"Checkpoint found: {ckpt_path}. Skipping training.")

        # Evaluate all variations side-by-side
        log.info("--> Loading Engine for Side-by-Side Evaluation...")
        ...
        if not split_queries:
            log.warning(f"No split queries found for {ds}. Skipping evaluation.")
            continue
            
        # Compute embeddings cleanly per split
        split_embs = {}
        ...
        all_results = {f"mlp_{loss}_tau_{tau:g}": {} for loss in LOSSES for tau in TAUS}
        ...
        for loss in LOSSES:
            for tau in TAUS:
                ...
                for split_name, queries in split_queries.items():
                    ...
                    res["method"] = f"mlp_{loss}_tau_{tau:g}"
                    all_results[f"mlp_{loss}_tau_{tau:g}"][split_name] = res
                ...
                if ds == "metaqa":
                    ...
                    for hop in sorted(hops.keys()):
                        ...
                        h_res["method"] = f"mlp_{loss}_tau_{tau:g}"
                        all_results[f"mlp_{loss}_tau_{tau:g}"][f"{split_name}_hop{hop}"] = h_res
        ...
        # Custom CSV mapping to bypass destroying the primary benchmark_results.csv
        csv_path = f"results/temp_ablation/{ds}_temp_ablation_results.csv"
        os.makedirs(os.path.dirname(csv_path), exist_ok=True)
        import csv
        from src.evaluation.benchmark_partition_selection import CSV_COLUMNS
        
        # Inject explicit tau tracking cleanly
        extended_cols = ["dataset", "method", "tau", "split"] + [c for c in CSV_COLUMNS if c not in ["dataset", "method", "split"]]
        
        with open(csv_path, "w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=extended_cols, extrasaction="ignore")
            writer.writeheader()
            for method, splits in all_results.items():
                _tau_val = float(method.split("_tau_")[1]) if "_tau_" in method else 0.07
                for split_name, metrics in splits.items():
                    row = {"dataset": ds, "method": method, "tau": _tau_val, "split": split_name}
                    row.update(metrics)
                    writer.writerow(row)
        ...
        json_path = f"results/temp_ablation/comparison_{ds}_temp.json"
        os.makedirs(os.path.dirname(json_path), exist_ok=True)
        with open(json_path, 'w') as f:
            json.dump(all_results, f, indent=4, default=safe_convert)

        volume.commit()
        log.info(f"\n  [Temp Ablation Engine] Benchmarks mapped cleanly. Exported exact CSV schema to {csv_path} and full JSON to {json_path}")
...
def ablation_main(dataset: str = "2wiki"):
    print(f"🚀 Pushing C-RAG Ablation tests to Modal cloud (A10G GPU). Dataset={dataset}...")
    ...
    for ds in ds_list:
        ...
        ckpt_dir = f"checkpoints/{ds}/temp_ablation"
        if os.path.exists(ckpt_dir):
            files = os.listdir(ckpt_dir)
            if any(f.endswith('.pth') for f in files):
                print(f"  ⬆️ Pushing local checkpoints {ckpt_dir} -> Cloud Volume /{ckpt_dir}")
                # We put the directory explicitly mapping the trailing slash logic identically
                subprocess.run([sys.executable, "-m", "modal", "volume", "put", "crag-data-volume", ckpt_dir, f"checkpoints/{ds}/temp_ablation"], capture_output=True, check=False)
...
if __name__ == "__main__":
    # If run natively via `python run_loss_eval.py`, act as the clean pipeline wrapper
    import subprocess
    import re

    log_filename = "pipeline_temp_ablation.log"
    cmd = [sys.executable, "-m", "modal", "run", "run_temp_eval.py"]
    if len(sys.argv) > 1:
    ...
    print(f"🚀 Launching Ablation Pipeline for {dataset}...")
