    ...
    for ds in datasets:
        ...
        for method in RERANKER_METHODS:
            ...
            for split_name in ["train", "val", "test"]:
                ...
                log.info(
                    f"  [{split_name}] R@1={summary.get('recall@1',0):.1f}% R@5={summary.get('recall@5',0):.1f}% "
                    f"GTR@10={summary.get('gt_recall@10',0):.1f}% MRR={summary.get('mrr',0):.1f}% "
                    f"FCov@20={summary.get('full_coverage@20',0):.1f}% FCov@100={summary.get('full_coverage@100',0):.1f}% "
                    f"L1={summary.get('avg_l1_latency_ms',0):.1f}ms L2={summary.get('avg_l2_latency_ms',0):.1f}ms "
                ...
                if ds == "metaqa":
                    ...
                    hop_indices = dd2(list)
                    for i, (q_node, _, _) in enumerate(queries):
                        hop = str(q_node.metadata.get("hop", "unknown"))
                        hop_indices[hop].append(i)

                    for hop in sorted(hop_indices.keys()):
                        idxs = hop_indices[hop]
                        hop_metrics = {k: [v[i] for i in idxs] for k, v in all_metrics.items()}
                        hop_lat = [latencies[i] for i in idxs]
                        hop_l1 = [l1_latencies[i] for i in idxs]
                        hop_l2 = [l2_latencies[i] for i in idxs]
                        hop_pool = [pool_sizes[i] for i in idxs]

                        h_summary = get_summary(hop_metrics, hop_lat, hop_l1, hop_l2, hop_pool, len(idxs))
                        method_results[f"{split_name}_hop{hop}"] = h_summary

                        log.info(
                            f"    ↳ Hop {hop} ({len(idxs)} queries): R@1={h_summary.get('recall@1',0):.1f}% "
                            f"R@5={h_summary.get('recall@5',0):.1f}% L1={h_summary.get('avg_l1_latency_ms',0):.1f}ms"
                        )
