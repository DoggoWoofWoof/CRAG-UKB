...
def sync_data_from_volume(datasets_to_run: list):
    ...
    for d in datasets_to_run:
        sync_targets.extend([
            f"checkpoints/{d}/losses_ablation",
            f"results/loss_ablation/{d}_loss_ablation_results.csv",
            f"results/loss_ablation/comparison_{d}_ablation.json"
        ])
...
LOSSES = ["info_nce_single", "info_nce_multi", "kl_div", "bce"]
...
def run_cloud_ablation(datasets=["2wiki"]):
    ...
    for ds in datasets:
        ...
        for loss in LOSSES:
            log.info(f"--> Ensuring {loss} model is trained...")
            ckpt_path = f"checkpoints/{ds}/losses_ablation/alignment_mlp_{loss}.pth"
            if not os.path.exists(ckpt_path):
                log.info(f"Model missing. Triggering training engine for {loss}...")
                train_ablation(dataset_name=ds, loss_type=loss, epochs=100) 
        ...
        all_results = {f"mlp_{loss}": {} for loss in LOSSES}
        ...
        for loss in LOSSES:
            ...
            for split_name, queries in split_queries.items():
                ...
                res["method"] = f"mlp_{loss}"
                all_results[f"mlp_{loss}"][split_name] = res
                ...
                if ds == "metaqa":
                    ...
                    for hop in sorted(hops.keys()):
                        ...
                        h_res["method"] = f"mlp_{loss}"
                        all_results[f"mlp_{loss}"][f"{split_name}_hop{hop}"] = h_res
        ...
        csv_path = f"results/loss_ablation/{ds}_loss_ablation_results.csv"
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            for method, splits in all_results.items():
                for split_name, metrics in splits.items():
                    row = {"dataset": ds, "method": method, "split": split_name}
                    row.update(metrics)
                    writer.writerow(row)
        ...
        json_path = f"results/loss_ablation/comparison_{ds}_ablation.json"
        os.makedirs(os.path.dirname(json_path), exist_ok=True)
        with open(json_path, 'w') as f:
            json.dump(all_results, f, indent=4, default=safe_convert)

        volume.commit()
        log.info(f"\n  [Loss Ablation Engine] Benchmarks mapped cleanly. Exported exact CSV schema to {csv_path} and full JSON to {json_path}")
...
def ablation_main(dataset: str = "2wiki"):
    ...
    for ds in ds_list:
        ...
        ckpt_dir = f"checkpoints/{ds}/losses_ablation"
        if os.path.exists(ckpt_dir):
            ...
            if any(f.endswith('.pth') for f in files):
                ...
                subprocess.run([sys.executable, "-m", "modal", "volume", "put", "crag-data-volume", ckpt_dir, f"checkpoints/{ds}/losses_ablation"], capture_output=True, check=False)
...
if __name__ == "__main__":
    ...
    log_filename = "pipeline_ablation.log"
    cmd = [sys.executable, "-m", "modal", "run", "run_loss_eval.py"]
    if len(sys.argv) > 1:
